...
import { loginIfNeeded, logout } from "./auth";

/**
 * Legacy login command - prefer using loginIfNeeded() for better reliability
 * @deprecated Use cy.loginIfNeeded() instead
 */
...
/**
 * Robust login helper that handles multiple authentication states
 * Automatically detects if user is already logged in and handles Keycloak flow
 */
Cypress.Commands.add(
  "loginIfNeeded",
  (options?: { useMfa?: boolean; timeout?: number }) => {
    loginIfNeeded(options);
  },
);
