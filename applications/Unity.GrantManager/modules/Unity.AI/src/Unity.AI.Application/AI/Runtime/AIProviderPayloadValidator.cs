...
namespace Unity.AI.Runtime
{
    public static class AIProviderPayloadValidator
    {
        ...
        public static AIResponseValidationResult ValidateFormWorksheetJson(string response)
        {
            if (!TryParseRootObject(response, out var root))
            {
                return AIResponseValidationResult.Invalid("Worksheet response was not valid JSON.");
            }

            foreach (var propertyName in new[] { "Name", "Title", "Version", "Published", "ReportColumns", "ReportKeys", "ReportViewName" })
            {
                var result = ValidateRequiredProperty(root, propertyName, "worksheet");
                if (!result.IsValid)
                {
                    return result;
                }
