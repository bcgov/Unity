...
namespace Unity.Flex.Web.Views.Shared.Components.QuestionYesNoWidget
{
    ...
    {
        public async Task<IViewComponentResult> InvokeAsync(Guid questionId, bool isDisabled, string? answer, string? yesValue, string? noValue, bool required = false, bool isHumanConfirmed = true, string? aiCitation = null, int? aiConfidence = null)
        {
            return View(await Task.FromResult(new QuestionYesNoViewModel() { QuestionId = questionId, IsDisabled = isDisabled, Answer = answer ?? string.Empty, YesValue = yesValue, NoValue = noValue, Required = required, IsHumanConfirmed = isHumanConfirmed, AICitation = aiCitation, AIConfidence = aiConfidence }));
        }
