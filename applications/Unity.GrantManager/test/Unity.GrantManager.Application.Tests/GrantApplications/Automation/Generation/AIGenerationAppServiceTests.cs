...
public class AIGenerationAppServiceTests(ITestOutputHelper outputHelper) : GrantManagerApplicationTestBase(outputHelper)
{
    ...
    [Fact]
    public async Task GenerateAttachmentSummariesAsync_Should_Queue_All_Attachments_When_None_Selected()
    {
        var queue = Substitute.For<IApplicationGenerationQueue>();
        var service = new AIGenerationAppService(
            queue,
            Substitute.For<IAIGenerationStatusReader>(),
            CreateFeatureGuard(),
    ...
    {
        var applicationId = Guid.NewGuid();
        var operationType = AIGenerationOperations.ApplicationAnalysis;
