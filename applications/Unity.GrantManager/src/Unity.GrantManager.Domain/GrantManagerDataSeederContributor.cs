    ...
    : IDataSeedContributor, ITransientDependency
{
    ...
    public async Task SeedAsync(DataSeedContext context)
    {
        ...
        ApplicationStatus? status1 = await _applicationStatusRepository.FirstOrDefaultAsync(s => s.StatusCode == GrantApplicationState.IN_PROGRESS);
        status1 ??= await _applicationStatusRepository.InsertAsync(
            new ApplicationStatus
            {
                StatusCode = GrantApplicationState.IN_PROGRESS,
