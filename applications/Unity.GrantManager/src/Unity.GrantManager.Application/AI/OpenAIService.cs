...
RULES
- Use only APPLICATION CONTENT, ATTACHMENT SUMMARIES, and SECTION as evidence.
- Do not invent missing application details.
- Return exactly one answer object per question ID in SECTION.
- Do not omit any question IDs from SECTION.
- Do not add keys that are not question IDs from SECTION.
- Each answer object must include: answer, citation, confidence.
...
- citation must be 1-2 complete sentences grounded in concrete evidence from provided inputs.
- If evidence is insufficient, give a conservative answer and state uncertainty in citation.
...
TASK
Using APPLICATION CONTENT, ATTACHMENT SUMMARIES, SECTION, OUTPUT, and RULES, answer only the questions in SECTION.";
