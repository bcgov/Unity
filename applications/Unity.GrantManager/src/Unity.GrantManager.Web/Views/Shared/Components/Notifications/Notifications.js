(function () {
    ...
    function loadList() {
        ...
        const absOffset = offsetType === 'Before' ? -Math.abs(offset) : Math.abs(offset);
        const offsetDays = triggerType === 'Date' ? absOffset : 0;

        const dateTypeValue = triggerType === 'Date' ? dateType : null;
        const applicationStatusId = triggerType === 'Event' ? (appStatusId || null) : null;
        const notificationRecipientCategory = triggerType === 'Event' ? recipientCategory : null;
        const notificationRecipientIdentifier = triggerType === 'Event' ? recipient : null;
        ...
        const body = {
            ...
            eventStatus: null,
            recipientCategory: notificationRecipientCategory,
            recipientIdentifier: notificationRecipientIdentifier
        };
