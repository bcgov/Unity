...
function approvedAmountUpdated(event) {
    ...
    const value = Number.parseFloat(input.value.replaceAll(/,/g, ''));
...
function runValidations() {
    ...
    $('#bulkApprovalForm input[name="BulkApplicationApprovals.Index"]').each(function () {
        itemCount++;
        let index = $(this).val();
        let approvedAmount = Number.parseFloat($('#bulkApprovalForm input[name="BulkApplicationApprovals[' + index + '].ApprovedAmount"]').val().replaceAll(/,/g, ''));
