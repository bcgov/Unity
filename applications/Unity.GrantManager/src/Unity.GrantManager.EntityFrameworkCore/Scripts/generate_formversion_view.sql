CREATE OR REPLACE PROCEDURE "Reporting".generate_formversion_view(IN correlation_id uuid)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    view_name TEXT;
    ...
    report_map_id UUID;
BEGIN
    -- Fetch the view name, mapping data, and ID from ReportColumnsMap for formversion provider
    SELECT "Id", "ViewName", "Mapping"::JSONB INTO report_map_id, view_name, mapping_data
    FROM "Reporting"."ReportColumnsMaps"
    WHERE "CorrelationId" = correlation_id AND "CorrelationProvider" = 'formversion';
    
    -- Check if we found the mapping
    IF mapping_data IS NULL OR report_map_id IS NULL THEN
        RAISE EXCEPTION 'No formversion mapping found for CorrelationId: %', correlation_id;
    END IF;
    
    -- Generate a view name if it's empty or null
    IF view_name IS NULL OR trim(view_name) = '' THEN
        view_name := 'formversion_view_' || replace(correlation_id::text, '-', '_');
    END IF;
    ...
    -- Get the data clause from the formversion data provider using the report map ID
    data_clause := "Reporting".get_formversion_data(correlation_id, report_map_id);
    ...
    RAISE NOTICE 'Successfully created formversion view: %', view_name;
END;
$procedure$;