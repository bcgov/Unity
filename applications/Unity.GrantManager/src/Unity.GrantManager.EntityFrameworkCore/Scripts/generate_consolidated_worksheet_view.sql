...
DECLARE
    ...
    report_map_id UUID;
BEGIN
    -- Fetch the view name, mapping data, and ID from ReportColumnsMap for worksheet_consolidated provider
    SELECT "Id", "ViewName", "Mapping"::JSONB INTO report_map_id, view_name, mapping_data
    FROM "Reporting"."ReportColumnsMaps"
    WHERE "CorrelationId" = form_id AND "CorrelationProvider" = 'worksheet_consolidated';
