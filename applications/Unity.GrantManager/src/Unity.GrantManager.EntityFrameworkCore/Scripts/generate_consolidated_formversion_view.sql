CREATE OR REPLACE PROCEDURE "Reporting".generate_consolidated_formversion_view(IN form_id uuid)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    view_name TEXT;
...
BEGIN
    ...
    IF mapping_data IS NULL OR report_map_id IS NULL THEN
        RAISE EXCEPTION 'No consolidated formversion mapping found for FormId: %', form_id;
    END IF;

    -- Generate a view name if it is empty or null
    IF view_name IS NULL OR trim(view_name) = '' THEN
        view_name := 'consolidated_formversion_view_' || replace(form_id::text, '-', '_');
    END IF;
    ...
    IF mapping_rows IS NULL OR jsonb_array_length(mapping_rows) = 0 THEN
        RAISE EXCEPTION 'No mapping rows found for consolidated formversion FormId: %', form_id;
    END IF;

    -- Get the data clause from the consolidated formversion data function
    data_clause := "Reporting".get_consolidated_formversion_data(form_id, report_map_id);
    ...
    RAISE NOTICE 'Successfully created consolidated formversion view: %', view_name;
END;
$procedure$;
