...
- **Deprecated reporting columns:** `Worksheet` and `Scoresheet` still carry `ReportColumns`/`ReportKeys`/`ReportViewName`, and both instance types carry `ReportData`. They fed the deprecated auto-generated reporting views, are no longer written by any code, and hold their last generated values until the planned Phase 2 migration drops them — see `documentation/reporting/reporting-auto-generated-views.md`. Reporting Configuration views read `WorksheetInstance.CurrentValue` and `Answer.CurrentValue` directly — see [flex-application-services.md](flex-application-services.md#reporting-integration).
