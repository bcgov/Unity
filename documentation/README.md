...
| `applications/Unity.GrantManager.Angular/**`<br>`AGM/src/Unity.GrantManager.Web/Controllers/AngularApp/**`<br>`AGM/src/Unity.GrantManager.Web/Pages/ConfigurationManagement/**` | [`angular-strangler-fig/architecture.md`](angular-strangler-fig/architecture.md) | The strangler-fig UI migration: the `/app` path split, the Angular app and its BFF controllers, auth continuity, shared theming, and local setup |
